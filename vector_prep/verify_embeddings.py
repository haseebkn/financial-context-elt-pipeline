...
def main():
    try:
        ...
        logger.info(
            "Collection verification", collection=COLLECTION_NAME, total_records=count
        )
        ...
        for idx in range(len(results["ids"][0])):
            ...
            print(
                f"    Cosine distance: {results['distances'][0][idx]:.4f}  (similarity: {1 - results['distances'][0][idx]:.4f})"
            )
