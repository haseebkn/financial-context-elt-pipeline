...
class AlpacaExtractor(BaseAPIClient):
    ...
    def __init__(self, config_path: str = "config/pipeline_config.yaml"):
        ...
        self.is_paper = (
            os.getenv(
                "ALPACA_IS_PAPER",
                str(self.alpaca_config.get("is_paper", settings.alpaca_is_paper)),
            ).lower()
            == "true"
        )
    ...
    def extract_orders(self) -> Generator[List[Dict[str, Any]], None, None]:
        ...
        self.logger.info(
            "Starting Alpaca orders extraction", start_time=start_time, limit=limit
        )
        ...
        while True:
            ...
            orders = self.execute_with_resilience(
                self.client.get_orders, filter=request_params
            )
            ...
            if len(orders) < limit:
                self.logger.info(
                    "Retrieved all available orders in final page", count=len(orders)
                )
                break
            ...
            if new_cursor <= after_cursor:
                self.logger.warning(
                    "Alpaca orders cursor failed to advance. Stopping pagination."
                )
                break
