...
class RawFileWriter:
    ...
    def __init__(self, base_dir: str):
        ...
        # Read AWS S3 details from settings
        ...
        if self.s3_bucket and not self.s3_bucket.startswith("your_"):
            try:
                ...
                logger.info(
                    "Successfully initialized AWS S3 Client for data lake sync",
    ...
    def _upload_to_s3(self, local_file_path: str, s3_key: str):
        ...
        try:
            logger.info(
                "Uploading raw record to AWS S3 data lake...",
                bucket=self.s3_bucket,
                key=s3_key,
            )
            self.s3_client.upload_file(local_file_path, self.s3_bucket, s3_key)
            logger.info(
                "Successfully synced record to AWS S3 data lake",
                bucket=self.s3_bucket,
                key=s3_key,
            )
    ...
    def write_record(
        ...
        try:
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(
                    envelope, f, indent=2, ensure_ascii=False, cls=DateTimeEncoder
                )
